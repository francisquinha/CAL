...
public:
	...
	double maxCost;///<the limit (not relevant on initialize, should be set by the methods later)
...
public:
	...
	/**@brief What do you expect? Finds a solution if there is one for the given BnB problem.
	 * (Note: The BnB by itself will not solve problem8, the initial city must be taken into account separately)
	 * @return list of pointers to the elements of the found solution. If no solution is found, will be returned an empty vector.
	 * */
	vector<T*> findSolution(double limitCost)
						{
		this->maxCost=limitCost;
	...
	/**
	 * brief try to find a approximate solution to problem 8 using a BnB approach. Work better if cost of the nodes (time spent on checkpoints) is
	 * greater than the edges cost (travel time between checkpoints)
	 * ... ... ... NOTE:WILL REMOVE START POINT NODE FROM NODES ... ... ...
	 * param StartPoint mem address of the start point (should be the same given to ordered_elements)
	 * param firtNodeCost method should return the max cost (or slightly above value) that could be spent travelling in and out of the initil node
	 * (note:could be max edge value *2 or even better if could get the sum of the 2 max edges connected to the origin)
	 * param limitCost the time limit to visit the given checkpoints
	 * return checkpoints that can be visited in any given order without exceeding the limit cost
	 * */
	vector<T*> problem8ApproximateSolve(T *StartPoint,double (T::*firtNodeCost)(void),double limitCost)
	{
		//will compare pointers, meaning they should point to the same element in memory
		for(int i = ordered_elements.size()-1; i>=0;i--){
			//cout<<"--"<< ordered_elements[i].second << "|||"<< StartPoint <<"--"<<"\n";
			if (ordered_elements[i].second==StartPoint)
			{
				ordered_elements.erase(ordered_elements.begin()+i);
				limitCost-=((*StartPoint).*firtNodeCost)();
			}
		}
		return findSolution(limitCost);
	}

	/**
	 * The same a the the overload description ut instead of receiving and comparing pointers, uses a copy
	 * of the object and compares the object with the operator ==
	 * */
	vector<T*> problem8ApproximateSolve(T StartPoint,double (T::*firtNodeCost)(void),double limitCost)
	{
		//will compare pointers, meaning they should point to the same element in memory
